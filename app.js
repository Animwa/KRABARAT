...
function getAdminRole() {
  ...
  return String(currentAdmin.role || "Admin").trim().toLowerCase();
}

function isSuperOrDaerah() {
  const role = getAdminRole();
  return role === "admin daerah" || role === "superadmin" || role === "admin";
}

function canWritePenyapaan() {
  // Hanya Admin Daerah / Superadmin yang bisa mengubah/menyimpan data penyapaan
...
function updateAdminUI() {
  ...
  if (currentAdmin) {
    ...
    if (role === "admin desa" && desaSelect && currentAdmin.scopeDesa) {
      ...
      const found = mk.find(m => String(m.Nama_Kelompok).trim().toLowerCase() === String(currentAdmin.scopeKelompok).trim().toLowerCase());
      if (found && desaSelect) {
      ...
      if (kelSelect) {
        kelSelect.value = currentAdmin.scopeKelompok;
...
function toggleLocalSapa(idKelompok, actionType) {
  // Hanya Admin Daerah yang bisa mengubah status penyapaan
